...
class GaussianModel:
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate(
            (xyz, normals, normals2, f_dc, f_rest, opacities, scale, rotation, f_asg),
            axis=1)
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter, voxel_visible_mask, use_filter=False):
        if use_filter:
            self.xyz_gradient_accum[voxel_visible_mask] += torch.norm(
                viewspace_point_tensor.grad[update_filter, :2], dim=-1,
                keepdim=True)
            self.denom[voxel_visible_mask] += 1
        else:
