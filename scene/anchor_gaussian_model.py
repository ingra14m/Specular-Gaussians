...
class AnchorGaussianModel:
    ...
    def __init__(self,
        ...
        self.mlp_color = AnchorSpecularNetwork(feature_dims=feat_dim).cuda()
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        points = self.voxelize_sample(points, voxel_size=self.voxel_size)  # init voxel center Equ (4)
        fused_point_cloud = torch.tensor(np.asarray(points)).float().cuda()
        # offsets = torch.zeros((fused_point_cloud.shape[0], self.n_offsets, 3)).float().cuda()
        offsets = ((torch.rand(
            (fused_point_cloud.shape[0], self.n_offsets, 3)) * 2 - 1) * self.voxel_size).float().cuda()
    ...
    def anchor_growing(self, grads, threshold, offset_mask):
        ...
        init_anchor = self.get_anchor
        init_feat = self._anchor_feat
        for i in range(self.update_depth):
            ...
            if candidate_anchor.shape[0] > 0:
                ...
                new_feat = scatter_max(new_feat, inverse_indices.unsqueeze(1).expand(-1, new_feat.size(1)), dim=0)[0][
                    remove_duplicates]
                # knn_res = knn_points(candidate_anchor[None], init_anchor[None], None, None, K=2)
                # knn_weights = 1 - F.normalize(knn_res.dists[0], dim=-1)
                # knn_idx = knn_res.idx[0]
                # knn_feat = init_feat[knn_idx]
                # new_feat = torch.sum(knn_weights[..., None] * knn_feat, dim=1)
